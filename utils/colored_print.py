...
_colored_print_compname = Logger().register_component(__file__)

# respect NO_COLOR
no_color_options = ["NO_COLOR"]
no_color = any([
    no_color_option in os.environ and os.environ[no_color_option] != ""
    for no_color_option in no_color_options
])
Logger().log(_colored_print_compname, logging.INFO, f"No color is set to {no_color}")
...
def colored_print(*args, ansi_color_str: str | ANSIColors, **kwargs) -> None:
    if no_color:
        ...
        with io.StringIO() as output_str:
            print(*args, file=output_str, **kwargs)
            print(f"{ansi_color_str}{output_str.getvalue()}{ANSIColors.ENDC}", end="")


def cprintf(*args, **kwargs):
    """Argument list same as print"""
    colored_print(*args, ansi_color_str=ColoredPrintSetting.MSG_COLOR_DICT[logging.CRITICAL], **kwargs)
...
def wprintf(*args, **kwargs):
    """Argument list same as print"""
    colored_print(*args, ansi_color_str=ColoredPrintSetting.MSG_COLOR_DICT[logging.WARNING], **kwargs)
...
def lprintf(level: str | int, *args, **kwargs):
    ...
    colored_print(*args, ansi_color_str=ColoredPrintSetting.MSG_COLOR_DICT[__check_level(level)], **kwargs)